...
describe('Single Attribute', () => {
    test('10 WEA', () => {
        ...
        for (const [type, scaling] of Object.entries(AttributeStatScaling[AttributeType.WeaponSkill])) {
            expect(char.stats[type as StatType].attribute).toEqual(num * scaling);
        }
    });
...
describe('Overlapping Attributes', () => {
    test('10 STR, 10 DEX - MeleeWeaponDamagePercent', () => {
        ...
        expect(char.stats[StatType.MeleeWeaponDamagePercent].attribute).toEqual((AttributeStatScaling.Strength[StatType.MeleeWeaponDamagePercent] ?? 0) * num + (AttributeStatScaling.Dexterity[StatType.MeleeWeaponDamagePercent] ?? 0) * num);
    });
    ...
    test('10 DEX, 10 WIS - Initiative', () => {
        ...
        expect(char.stats[StatType.Initiative].attribute).toEqual((AttributeStatScaling.Dexterity[StatType.Initiative] ?? 0) * num + (AttributeStatScaling.Wisdom[StatType.Initiative] ?? 0) * num);
    });
    ...
    test('10 CON, 10 INT - Armour', () => {
        ...
        expect(char.stats[StatType.Armour].attribute).toEqual((AttributeStatScaling.Constitution[StatType.Armour] ?? 0) * num + (AttributeStatScaling.Intelligence[StatType.Armour] ?? 0) * num);
    });
    ...
    test('10 INT, 10 WIS - ManaOnHit', () => {
        ...
        expect(char.stats[StatType.ManaOnHit].attribute).toEqual((AttributeStatScaling.Intelligence[StatType.ManaOnHit] ?? 0) * num + (AttributeStatScaling.Wisdom[StatType.ManaOnHit] ?? 0) * num);
    });
});